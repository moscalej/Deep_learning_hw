...
FL1 = Dropout(0.4)(LeakyReLU()(Dense(64, kernel_regularizer=keras.regularizers.l2(weight_decay))(Flatten()(pull_4))))
...
def run(x_train,y_train,x_test,y_test):
    tbCallBack = keras.callbacks.TensorBoard(log_dir='./Code/logs/run2/', histogram_freq=0, batch_size=32, write_graph=True,
                                             write_grads=False, write_images=False, embeddings_freq=0,
                                             embeddings_layer_names=None, embeddings_metadata=None)

    history_fully = model.fit(x_train, y_train, epochs=200, batch_size=1024, validation_data=(x_test, y_test),callbacks=[tbCallBack])
